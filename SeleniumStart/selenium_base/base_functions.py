...
class SeleniumBase:
    ...
    def __init__(self, driver):  # функция для инициализирования нашего браузера ,("self"-функция принимает сама себя) и браузер
        ...
        self.wait = WebDriverWait(driver, 15, 0.3)  # команда задержки (драйвер, время, задержка)
    ...
    def is_visiable(self, find_by: str, locator: str, locator_name: str = None) -> WebElement:  # функция для того что бы проверить отображается ли наш элемент СРАЗУ(то что есть в коде) или нет и какие типы данных она принимает и возвращает
