...
# создаем функцию где мы получаем nav_links
    ...
    def get_nav_links(self) -> List[WebElement]:                   # функция передает саму себя и возвращает список(потому что элементов несколько) WebElement
        return self.are_visiable('css', self.nav_links, 'Header Navigation links')  # прописываем команду возврата  указываем по какому принципу она будет возвращать данные с функцией are_visiable из бейс.пай("указали по чему будем искать",локатор self.nav_links, название того что ищем,)
