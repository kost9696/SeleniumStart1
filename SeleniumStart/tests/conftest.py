...
def get_webdriver(get_chrome_options):                                                              # фунцкия которая вызывает вебдрайвер
    ...
    driver = webdriver.Chrome(executable_path="E:\\PycharmProjects\\SeleniumStart\\ChromeDriver\\chromedriver.exe", options = options)    # создаем переменную driver,после передаем туда значение пути к вебдрайверу и передаем опции котроые прописывали ранее
...
def setup(request, get_webdriver):                                                                           # создаем фунцкию дляобьявления url,тоже самое что и driver.get(url=url)
    ...
    url = 'https://rozetka.com.ua/'                                                                  # указываем url
